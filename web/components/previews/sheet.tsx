...
function SheetGrid({ ws, utils }: { ws: WorkSheet; utils: Parsed["utils"] }) {
  ...
  if (!grid) return <div className="p-6 text-caption text-drive-muted">This sheet is empty.</div>;
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col">
      {clipped && (
        <div className="shrink-0 px-3 py-1.5 text-caption text-drive-muted bg-drive-sidebar border-b border-drive-border">
      ...
      <div className="flex-1 min-h-0 overflow-auto bg-white">
        <table className="border-separate border-spacing-0 text-[13px] leading-5 text-drive-text" style={{ tableLayout: "fixed", width: 44 + grid.widths.reduce((a, b) => a + b, 0) }}>
          <colgroup>
            <col style={{ width: 44 }} />
          ...
          <thead>
            <tr>
              ...
              {grid.widths.map((w, c) => (
                <th key={c} className="sticky top-0 z-10 bg-[#f8f9fa] border-r border-b border-[#c0c0c0] font-normal text-[11px] text-drive-muted h-5 overflow-hidden">
                  {w ? colName(c) : null}
                </th>
              ))}
            </tr>
            ...
            {Array.from({ length: grid.rows }, (_, r) => {
              ...
              return (
                <tr key={r} style={hpx !== undefined ? { height: hpx } : undefined}>
                  <th className="sticky left-0 z-10 bg-[#f8f9fa] border-r border-b border-[#c0c0c0] font-normal text-[11px] text-drive-muted text-center h-[21px]">{r + 1}</th>
                  {grid.widths.map((_, c) => {
                    const key = `${r},${c}`;
                    ...
                    return (
                      <td
                        ...
                        className={clsx(
                          ...
                          formulaOnly && "text-drive-muted italic",
...
function Workbook({ name, data }: { name: string; data: ArrayBuffer }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div role="tablist" className="shrink-0 flex gap-0.5 overflow-x-auto border-t border-drive-border bg-drive-sidebar px-2">
        {visible.map((i) => (
          <button
            key={i}
            role="tab"
            ...
            onClick={() => setActive(i)}
            className={clsx(
              "shrink-0 max-w-[180px] truncate px-3 py-1.5 text-caption border-b-2 -mt-px",
              i === active ? "bg-white border-drive-accent text-drive-accent font-medium" : "border-transparent text-drive-muted hover:bg-drive-hover",
            )}
