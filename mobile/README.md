...
| `android/…/agent/Router.java` | decides what a turn is before the index is touched: small talk, out of scope ("book a table for 4"), call report, or a file question; out-of-scope conversations stay out. `RouterTest` replays every user turn of the Schema-Guided Dialogue corpus (`test/resources/sgd-user-turns.tsv.gz`, CC BY-SA 4.0) — none may reach the index — plus the in-domain suites |
