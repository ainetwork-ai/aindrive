...
import { issueWalletCert, resolvePerson, signLink, type Cert } from "@/shared/willow/cert";
...
export async function walletCertFromLogin(o: { message: string; signature: string; address: string; userId: string; label: string }): Promise<Cert | null> {
  ...
  const cert = await issueWalletCert({ deviceKey: fromHex(m[1]), userId: o.userId, label: o.label.slice(0, 80), at: nowMicros(), address: o.address, message: o.message, signature: o.signature, link }, attestationKey());
  // only a cert every peer can verify (a smart wallet's ERC-1271 signature cannot be checked
  // offline): otherwise none, and the browser gets an attested cert instead (review C2)
  return (await resolvePerson(m[1], [cert], [], trust()))?.strength === "wallet" ? cert : null;
}
