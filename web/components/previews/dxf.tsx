    ...
    <div className="relative h-full min-h-[320px] bg-white">
      ...
      {status.state === "ready" && (
        <div className="absolute top-2 right-2 flex flex-col items-end gap-2">
          <div className="flex gap-1 rounded-full bg-white/90 shadow-e1 border border-drive-border p-1">
            <button type="button" onClick={() => viewerRef.current && fit(viewerRef.current)} className="p-1.5 rounded-full hover:bg-drive-hover text-drive-muted" title="Fit to view" aria-label="Fit to view">
              <Maximize className="w-4 h-4" />
            </button>
            {layers.length > 1 && (
              <button type="button" onClick={() => setShowLayers((v) => !v)} className={clsx("p-1.5 rounded-full hover:bg-drive-hover", showLayers ? "text-drive-accent" : "text-drive-muted")} title="Layers" aria-label="Layers" aria-pressed={showLayers}>
                <Layers className="w-4 h-4" />
              </button>
            )}
          </div>
          {showLayers && (
            <ul className="max-h-64 w-48 overflow-auto rounded-lg bg-white/95 shadow-e1 border border-drive-border py-1 text-caption text-drive-text">
              {layers.map((l) => (
                <li key={l.name}>
                  <label className="flex items-center gap-2 px-3 py-1 hover:bg-drive-hover cursor-pointer">
                    <input type="checkbox" checked={l.visible} onChange={(e) => toggleLayer(l.name, e.target.checked)} />
                    <span className="w-3 h-3 rounded-sm border border-drive-border shrink-0" style={{ background: `#${l.color.toString(16).padStart(6, "0")}` }} />
