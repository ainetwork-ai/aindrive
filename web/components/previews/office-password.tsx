...
import { decryptOoxml, isEncryptedOoxml, OfficePasswordError } from "@/lib/office-crypto";
  ...
  return (
    <div className="h-full min-h-[300px] flex items-center justify-center p-6">
      <form onSubmit={submit} className="w-full max-w-xs flex flex-col items-center text-center">
        ...
        <div className="mt-3 text-body font-medium text-drive-text max-w-full truncate" title={name}>{name}</div>
        <p className="mt-1 text-caption text-drive-muted">This file is password-protected. Enter the password to preview it.</p>
        <input
          type="password"
          autoFocus
          autoComplete="off"
          ...
          className="mt-4 w-full rounded-lg border border-drive-border px-3 py-2 text-body"
        />
        {error && <p role="alert" className="mt-2 text-caption text-red-600">{error}</p>}
        <button
          type="submit"
          disabled={!password || busy}
          className="mt-3 w-full rounded-lg bg-drive-accent text-white py-2 text-body hover:bg-drive-accentHover disabled:opacity-60 flex items-center justify-center gap-2"
        >
