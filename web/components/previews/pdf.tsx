...
function PasswordPrompt({ name, incorrect, onSubmit }: { name: string; incorrect: boolean; onSubmit: (pw: string) => void }) {
  ...
  return (
    ...
    >
      <Lock className="w-10 h-10 text-drive-muted" />
      <div className="text-body text-drive-text max-w-xs truncate" title={name}>{name}</div>
      <p className="text-caption text-drive-muted">This PDF is password-protected.</p>
      <div className="flex gap-2 w-full max-w-xs">
        <input
          ...
          className="flex-1 min-w-0 h-9 px-3 rounded-md border border-drive-border text-body focus:outline-none focus:ring-2 focus:ring-drive-accent"
        />
        <button type="submit" disabled={!value} className="h-9 px-3 rounded-md bg-drive-accent text-white text-body disabled:opacity-50">
          Open
        </button>
      </div>
      {incorrect && <p className="text-caption text-red-600">Incorrect password. Try again.</p>}
    </form>
  );
}
...
function PdfPages({ doc, aspect }: { doc: PDFDocumentProxy; aspect: number }) {
  ...
  const btn = "p-1.5 rounded-md text-drive-muted hover:bg-drive-hover hover:text-drive-text disabled:opacity-40";
  return (
    <div className="h-full flex flex-col bg-drive-bg">
      <div className="flex items-center justify-between gap-2 px-3 py-1.5 border-b border-drive-border bg-white text-caption text-drive-muted">
...
function PdfPage({
  ...
  return (
    <div
      ...
      className="pdf-page relative bg-white shadow-e2 shrink-0"
    ...
    >
      ...
      {failed && (
        <div className="absolute inset-0 flex items-center justify-center text-caption text-drive-muted">
...
function PdfTextLayerStyles() {
  return <style href="aindrive-pdf-text-layer" precedence="default">{TEXT_LAYER_CSS}</style>;
}
