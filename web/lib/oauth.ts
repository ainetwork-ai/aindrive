 ...
 *
 * Scope wire format: "drive:read" | "drive:write" ↔ McpScope "read"|"write".
 * The drive is identified by the RFC 8707 `resource` = mcpUrlFor(driveId).
 */
...
export const SCOPES_SUPPORTED = ["drive:read", "drive:write"] as const;
...
export function validateAuthorize(p: AuthorizeParams): { ok: true; value: ValidAuthorize } | { ok: false; error: string } {
  ...
  if (!driveId) {
    return { ok: false, error: "Missing or unknown `resource` — use the drive's MCP URL (…/mcp/d/<driveId>)." };
  }
  return {
    ok: true,
    value: {
      ...
      driveId,
      requestedScope: parseScope(p.scope),
      state: p.state ?? null,
    },
  };
}
...
export function redeemCode(opts: {
  ...
  if (row.client_id !== opts.clientId || row.redirect_uri !== opts.redirectUri) return null;
  if (!/^[A-Za-z0-9\-._~]{43,128}$/.test(opts.codeVerifier)) return null;
  if (pkceS256(opts.codeVerifier) !== row.code_challenge) return null;
