...
agent (`cli/` → `desktop/cli/aindrive.mjs`) with the app's own runtime as Node,
so pairing, sign-in and serving behave exactly like `npm i -g aindrive`.

| File | Role |
|------|------|
| `src/main.js` | Electron main: window, menu-bar icon, folder picker, `aindrive://share` links, open at login, IPC |
| `src/agents.js` | one CLI process per folder; state from its output (`parseLine`); restarts a paired folder that dies. No Electron imports — unit-tested |
...
| `scripts/build-mac.mjs` | `.app` + `.dmg` per arch, from Linux or macOS (see below) |
...
- **Electron ↔ better-sqlite3 ABI.** The CLI needs better-sqlite3, loaded by
  Electron-as-Node, so the prebuilt binary must match Electron's ABI *and*
