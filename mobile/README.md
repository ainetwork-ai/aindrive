...
| `android/…/llm/Summarizer.java` | the only LLM on the phone: Qwen2.5-1.5B-Instruct (8-bit) on MediaPipe LLM Inference, used solely to summarise transcripts for the call report (`assets/llm/models.json`, ~1.6 GB, optional — without it the report shows topic words). The report spends at most 30 s writing new summaries (`CallReport.SUMMARY_BUDGET_MS`; cached ones are free), the rest keep topic words, and the report card offers **Better summaries with aindrive-cloud**: the report's markdown notes (text, never the audio) as a handoff — links plus the MCP view of just those notes — when that agent is switched on |
